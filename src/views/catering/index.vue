...
import { reqCatering, reqCateringCount } from '@/api/tourism/food/index.ts';
...
const getTransportation = async () => {
  const result = await reqCatering(pageNo.value, limit.value);
  if (result.code == 200) {
    console.log(result);
...
const getTransportationCount = async () => {
  const result = await reqCateringCount();
  if (result.code == 200) {
